...
THRESHOLD: float = 0.1  # percentage we'll check in either direction
PENALTY: float = 1.005  # the extra badness per piece
...
def jig(w: float, h: float, n: int, b: int = 0) -> None:
    ...
    for dis_range in [up_range, down_range]:
        ...
        for i in dis_range:
            ...
            if yes == 1:
                ...
                print(
                    f"{best_n} pieces in {best_n_sides} (grid ratio {round(best_n_ratio, 4)}) needs piece ratio {round(piece_ratio, 4)}")
                if b == 1:
...
def jig_v0(w: float, h: float, n: int, b: int = 0) -> None:
    # percentage we'll check in either direction
    threshold = 0.1
...
if __name__ == '__main__':
    # TODO maybe change to an argument parser (with python module argparse)
